...
import { Result4Client } from '../utils/slot_utils'
axios.defaults.timeout = 5000

import * as EUtil from '../utils/engine_utils'
export const OnRequest_GameService = async function (req: any, res: Response) {
  var param: any = req.body
  ...
  var result = Result4Client(player.currentApi)
...
async function HandleBalance(param: any, player: any, user: any) {
  ...
  if (action == 'doSpin' || action == 'doCollect' || action == 'doCollectBonus') {
    ...
    if (debit > 0 || credit > 0) {
      ...
      if (global.logConfig.show_index) {
        logStr += `, (   :${player.curIndex - player.lastJackpotIndex}/${
          player.nextJackpot
        },    :${player.callHistId})`
      }
      ...
      if (global.logConfig.show_rtp) {
        logStr += `, (${Math.floor(credit)}/${debit}, player:${
          player.totalCredit
        }/${player.totalDebit}, ${player.realRtp}%,    :${user.totalCredit}/${
          user.totalDebit
        }, RTP:${user.realRtp}/${user.targetRtp})`
      }
      if (player.logInfo) {
...
async function SaveReplay(param: any, player: any, Replay: any) {
  ...
  player.replayLogList.push({
    cr: Buffer.from(Result4Client(param), 'utf8').toString('base64'),
    sr: Buffer.from(Result4Client(lastPattern), 'utf8').toString('base64'),
  })
...
async function SaveHistory(req: any, user: any, player: any) {
...
function HandleError(msg: string, res: any) {
  var obj = {
    action: 'error',
    msg: msg,
  }

  var result = Result4Client(obj)
  return res.send(result)
}

async function checkAvailable(req: any, res: any) {
  ...
  if (!game) {
    return HandleError('GAME_NOT_ALLOWED', res)
  }

  const user = await User.findOne({ where: { token } })
  if (!user) {
    return HandleError('INVALID_TOKEN', res)
  }
  ...
  if (!(player instanceof Player)) {
    player = await Player.findOrCreate({
      where: { gameCode, userCode: user.email },
    })
    if (player) {
      Object.assign(player, {
        nextJackpot: Math.floor(Math.random() * 20) * 7 + 60,
      })
    }
  }
  ...
  if (
    req.body.action != 'doInit' &&
    player.machine.currentGame == 'BASE' &&
    player.machine.tumbleStatus != 'TUMBLE'
  ) {
    if (Number(totalBet) > user.balance) {
      //console.info(
      //   `                             :         :${user.login}       :${user.balance},       : ${totalBet} `
      // );
      if (user.lang == 'en') {
