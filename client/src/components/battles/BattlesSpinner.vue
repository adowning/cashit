    ...
    <div class="spinner-inner">
      ...
      <div v-for="(bet, index) in battlesGetBets" v-bind:key="index" class="inner-element">
        <transition name="fade" mode="out-in">
          <div
            v-if="
              ['created', 'countdown'].includes(battlesGameData.game.state) === true && bet === null
            "
          ...
          <div
            v-else-if="['pending', 'rolling'].includes(battlesGameData.game.state) === true"
            class="element-wheel"
          >
            <BattlesReel
          ...
          >
            ...
            <div class="completed-action">
              <button
                v-if="index === 0 && authUser.user !== null && authUser.user._id === bet.user._id"
            ...
            <div class="box-inner">
              <IconGroupGradient
                v-if="
                  ...
                  (battlesGameData.game.mode === 'team' && (index === 0 || index === 2))
                "
              />
...
<script setup>
  ...
  import BattlesReel from '@/components/battles/BattlesReel.vue'
  import { reactive, ref, computed, watch } from 'vue'

  // Data
  const battlesRunning = ref(false)
  const battlesReelsPosRepeater = ref(null)
  const battlesReelsPos = ref(20)
  const battlesReels = reactive({
    1: [],
    2: [],
    3: [],
    4: [],
  })
  const battlesReelStyle = reactive({
  ...
  const battlesGetItemsFormated = function (items) {
    let pos = 0
  ...
  const battlesGetBoxItems = function (box) {
    let items = []

    for (let item of battlesGetItemsFormated(box.items)) {
  ...
  const battlesGetOutcomeItem = function (outcome, items) {
    let outcomeItem = null
    let pos = 0

    for (const item of battlesGetItemsFormated(items)) {
      pos = pos + item.tickets
      if (outcome <= pos) {
        outcomeItem = item
        break
  ...
  const battlesAddReels = function () {
    let items = battlesGetBoxItems(
      this.battlesGetBoxes[this.battlesGameData.game.bets[0].outcomes.length - 1]
    )
    battlesReels = { 1: [], 2: [], 3: [], 4: [] }
  ...
  const battlesGetReelsPos = function () {
    const offset =
    ...
    if (battlesReelsPos.value !== pos) {
      battlesReelsPos.value = pos

      if (battlesRunning.value === true && this.soundBattles === 1) {
    ...
    battlesReelsPosRepeater.value = requestAnimationFrame(battlesGetReelsPos)
  }

  const battlesCreateButton = function () {
  ...
  watch(
    battlesGameData,
    function (data, dataOld) {
      if (battlesGameData.game.state === 'rolling') {
        battlesAddReels()
        battlesGetReelsPos()
        battlesRunning.value = true

        for (const [index, bet] of battlesGameData.game.bets.entries()) {
          battlesReelStyle = {
            transform: 'translateX(0px) translateY(-2416.5px)',
            transition: 'none',
          }

          battlesReels[index + 1][60] = battlesGetOutcomeItem(
            bet.outcomes[bet.outcomes.length - 1],
            this.battlesGetBoxes[bet.outcomes.length - 1].items
          )

          setTimeout(() => {
            const timeEnding = new Date(battlesGameData.game.updatedAt).getTime() + 5000
            ...
            battlesReelStyle = {
              transform:
                ...
                (7364 + (105 / 8) * Math.floor(Math.random() * (7 - 1 + 1)) + 1) +
                'px)',
            ...
            setTimeout(() => {
              ...
              cancelAnimationFrame(battlesReelsPosRepeater.value)
              setTimeout(() => {
                battlesRunning.value = false
      ...
      } else if (battlesGameData.game.state === 'completed') {
        if (this.soundBattles === 1) {
...
<style scoped>
  ...
  .battles-spinner .info-inner {
    ...
    color: #ffffff;
    background:
      radial-gradient(60% 60% at 50% 50%, rgba(10, 238, 179, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
      #021524;
    clip-path: polygon(
      10px 0,
      calc(100% - 10px) 0,
  ...
  .battles-spinner .box-inner {
    ...
    align-items: center;
    background:
      radial-gradient(50% 50% at 50% 50%, rgba(10, 238, 179, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
      #051f33;
    clip-path: polygon(
      5px 0,
      calc(100% - 5px) 0,
