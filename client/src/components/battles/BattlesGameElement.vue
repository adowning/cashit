      ...
      <div class="inner-players">
        <div v-for="(bet, index) in battlesGetBets" v-bind:key="index" class="players-element">
          <div
            ...
            v-bind:class="[
              'user-' +
                (bet === null ? 'empty' : bet.bot === true ? 'bot' : battlesGetRank(bet.user)),
          ...
          <div
            v-if="index + 1 !== game.playerCount && (game.mode !== 'team' || index === 1)"
          ...
          <div class="amount-inner">
            <div class="inner-effective">
              ...
              <div class="effective-value">
                <span>{{
                  battlesFormatValue(
                    game.amount - Math.floor((game.amount * game.options.funding) / 100)
                  ).split('.')[0]
                }}</span
                >.{{
                  battlesFormatValue(
...
<script setup>
  ...
  import IconTerminalGradient from '@/components/icons/IconTerminalGradient.vue'
  import IconCursedGradient from '@/components/icons/IconCursedGradient.vue'
  import IconVersusGradient from '@/components/icons/IconVersusGradient.vue'
  import IconGroupGradient from '@/components/icons/IconGroupGradient.vue'
  import IconSpinner from '@/components/icons/IconSpinner.vue'
  import BattlesGameBoxes from '@/components/battles/BattlesGameBoxes.vue'
  ...
  // Computed
  const battlesGetMode = computed(() => {
    let mode = '1v1'

    if (props.game.mode === 'group') {
      mode = 'Group'
    } else if (props.game.mode === 'team') {
      mode = '2v2'
    } else if (props.game.playerCount === 3) {
      mode = '1v1v1'
    } else if (props.game.playerCount === 4) {
      mode = '1v1v1v1'
    }
  ...
  const battlesGetOption = computed(() => {
    let option = ''

    if (props.game.options.cursed === true && props.game.options.terminal === false) {
      option = 'CURSED'
    } else if (props.game.options.cursed === false && props.game.options.terminal === true) {
      option = 'TERMINAL'
    }
  ...
  const battlesGetBets = computed(() => {
    let bets = []

    for (let bet = 0; bet < props.game.playerCount; bet++) {
      const index = props.game.bets.findIndex((element) => element.slot === bet)

      bets.push(index !== -1 ? props.game.bets[index] : null)
    }

    return bets
  })

  const battlesGetBoxes = computed(() => {
    let boxes = []

    for (const box of props.game.boxes) {
  ...
  // Methods
  const battlesFormatValue = function (value) {
  ...
  const battlesGetRank = function (user) {
  ...
  const battlesGetLevelColor = function (user) {
    let color = ''
  ...
  const battlesJoinButton = function () {
    ...
    const data = { gameId: props.game._id, slot: props.game.bets.length }
...
<style scoped>
  ...
  .battles-game-element .info-type:before {
    background:
      linear-gradient(0deg, rgba(10, 39, 63, 0.35) 0%, rgba(10, 39, 63, 0.35) 100%),
  ...
  .battles-game-element .type-inner {
    ...
    color: #5e768e;
    background:
      radial-gradient(80% 80% at 50% 50%, rgba(46, 62, 202, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
  ...
  .battles-game-element.element-group .type-inner {
    background:
      linear-gradient(223deg, rgba(0, 255, 194, 0.25) 0%, rgba(0, 170, 109, 0.25) 100%),
      linear-gradient(223deg, rgba(5, 29, 48, 0.35) 0%, rgba(31, 99, 153, 0) 100%),
      repeating-linear-gradient(
        -55deg,
        rgba(255, 255, 255, 0.03),
  ...
  .battles-game-element.element-cursed .option-inner {
    background:
      linear-gradient(230deg, rgba(242, 64, 52, 0.15) 0%, rgba(121, 44, 0, 0.15) 100%),
      linear-gradient(223deg, rgba(5, 29, 48, 0.35) 0%, rgba(31, 99, 153, 0) 100%),
      repeating-linear-gradient(
        -55deg,
        rgba(255, 255, 255, 0.03),
  ...
  .battles-game-element.element-terminal .option-inner {
    background:
      linear-gradient(216deg, rgba(234, 214, 33, 0.15) 0%, rgba(255, 142, 38, 0.15) 100%),
      linear-gradient(223deg, rgba(5, 29, 48, 0.35) 0%, rgba(31, 99, 153, 0) 100%),
      repeating-linear-gradient(
        -55deg,
        rgba(255, 255, 255, 0.03),
  ...
  .battles-game-element.element-terminal .option-inner {
    background:
      linear-gradient(252deg, rgba(250, 159, 37, 0.15) 0%, rgba(121, 44, 0, 0.15) 100%),
      linear-gradient(223deg, rgba(5, 29, 48, 0.35) 0%, rgba(31, 99, 153, 0) 100%),
      repeating-linear-gradient(
        -55deg,
        rgba(255, 255, 255, 0.03),
  ...
  .battles-game-element .separator-inner {
    ...
    align-items: center;
    background:
      radial-gradient(78% 78% at 50% 50%, rgba(10, 238, 179, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
      #051f33;
    clip-path: polygon(
      4px 0,
      calc(100% - 4px) 0,
  ...
  .battles-game-element button.button-waiting .button-inner {
    background:
      linear-gradient(255deg, rgba(0, 255, 194, 0.15) 0%, rgba(0, 170, 109, 0.15) 100%), #051f35;
  }
