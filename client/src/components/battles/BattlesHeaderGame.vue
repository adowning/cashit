    ...
    <div class="game-info">
      <div
        v-if="
          ...
          ['created', 'countdown', 'pending', 'completed'].includes(battlesGameData.game.state) ===
            true
        "
        class="info-text"
      >
      ...
      <div v-else class="info-running">
        ...
        <div class="running-round">Round {{ battlesGetRound }}</div>
      </div>
...
<script setup>
  ...
  // Computed
  const battlesGetAmount = computed(() => {
    let amount = 0
  ...
  const battlesGetState = computed(() => {
  ...
  const battlesGetRound = computed(() => {
    let round = 1
  ...
  const battlesGetLink = computed(() => {
    let link = 'hypedraft.com/battles/'

    if (this.battlesGameData.game !== null) {
      link = 'rblxroll.com/battles/' + this.battlesGameData.game._id
    }
  ...
  const battlesFairButton = function () {
  ...
  const battlesCopyButton = function (value) {
...
<style scoped>
  ...
  .battles-header-game button.button-sound {
    ...
    border-radius: 5px;
    background:
      radial-gradient(70% 70% at 50% 50%, rgba(152, 151, 193, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
      #031b2e;
