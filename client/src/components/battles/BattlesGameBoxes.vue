...
<script setup>
  import IconCheckGradient from '@/components/icons/IconCheckGradient.vue'
  import { reactive, computed } from 'vue'

  // Data
  const unboxImagePath = reactive(process.env.VUE_APP_BACKEND_URL)
  ...
  const battlesGetRound = computed(() => {
    ...
    if (
      props.game !== null &&
      props.game.bets[0].outcomes !== undefined &&
      props.game.bets[0].outcomes.length >= 1
    ) {
      round = props.game.bets[0].outcomes.length
    }
  ...
  const battlesGetBoxes = computed(() => {
    ...
    return boxes.slice(battlesGetRound.value - 1, battlesGetRound.value + 11)
  })

  // Methods
  const battlesBoxButton = function (box) {
