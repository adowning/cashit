    ...
    <div class="element-inner">
      ...
      <div class="inner-price">
        ...
        <div class="price-value">
          <span>{{ unboxFormatValue(item.item.amountFixed).split('.')[0] }}</span
          >.{{ unboxFormatValue(item.item.amountFixed).split('.')[1] }}
        </div>
      </div>
    </div>
  </div>
...
<script setup>
  ...
  // Methods
  const unboxFormatValue = function (value) {
...
<style scoped>
  ...
  .battles-item-element.element-red .element-inner {
    background: radial-gradient(80% 80% at 50% 50%, rgb(218, 50, 50, 0.2) 0%, rgba(0, 0, 0, 0) 100%),
      linear-gradient(
        223deg,
        rgba(5, 29, 48, 0.35) 0%,
        rgba(31, 99, 153, 0.09) 50%,
