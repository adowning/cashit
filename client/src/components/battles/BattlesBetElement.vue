    ...
    <div class="element-top">
      <div class="top-inner">
        <div v-if="bet !== null" class="inner-info">
          ...
          <div class="info-amount">
            ...
            <div class="amount-value">
              <span>{{ battlesFormatValue(battlesGetOutcomeAmount).split('.')[0] }}</span
        ...
        <div v-else class="inner-actions">
          ...
          <button
            v-on:click="
              authUser.user !== null && authUser.user._id === battlesGameData.game.bets[0].user._id
                ? battlesBotButton()
                : battlesJoinButton()
            "
            class="button-join"
          ...
          >
            <div class="button-inner">
              <transition name="fade" mode="out-in">
                <ButtonLoading
                  v-if="socketSendLoading === 'BattlesJoin' || socketSendLoading === 'BattlesCall'"
                />
...
<script setup>
  ...
  import BattlesItemElement from '@/components/battles/BattlesItemElement.vue'
  import { computed } from 'vue'

  // Props
  const props = defineProps(['pos', 'bet'])

  // Computed
  const battlesGetBotName = computed(() => {
    let name = 'Redcliff'
  ...
  const battlesGetOutcomeItems = computed(() => {
    let items = []

    if (props.bet !== null) {
      for (const [index, outcome] of (this.battlesGameData.game.state === 'completed'
        ? props.bet.outcomes
        : props.bet.outcomes.slice(0, -1)
      ).entries()) {
        let pos = 0

        for (const item of this.battlesGetItemsFormated(battlesGetBoxes.value[index].items)) {
  ...
  const battlesGetOutcomeAmount = computed(() => {
    let amount = 0

    for (const item of battlesGetOutcomeItems.value) {
  ...
  const battlesBotButton = function () {
  ...
  const battlesJoinButton = function () {
    ...
    const data = { gameId: this.battlesGameData.game._id, slot: props.pos }
...
<style scoped>
  ...
  .battles-bet-element.element-winner .top-inner {
    background:
      linear-gradient(223deg, rgba(1, 232, 171, 0.2) 0%, rgba(0, 0, 0, 0) 50%),
      linear-gradient(
        223deg,
        rgba(5, 29, 48, 0.35) 0%,
        rgba(31, 99, 153, 0.1) 50%,
  ...
  .battles-bet-element.element-lost .top-inner {
    background:
      linear-gradient(223deg, rgba(235, 42, 42, 0.2) 0%, rgba(0, 0, 0, 0) 50%),
      linear-gradient(
        223deg,
        rgba(5, 29, 48, 0.35) 0%,
        rgba(31, 99, 153, 0.1) 50%,
