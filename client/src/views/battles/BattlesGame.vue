<template>
  <div
    ...
    v-bind:class="[
      ...
      'game-' + (battlesGameData.game !== null ? battlesGameData.game.state : ''),
    ]"
  >
    <transition name="fade" mode="out-in">
      <div
        v-if="socketBattles.connected === false || battlesGameData.loading === true"
        class="game-loading"
        key="loading"
      >
...
<script setup>
  import BattlesSpinner from '@/components/battles/BattlesSpinner.vue'
  import BattlesBetElement from '@/components/battles/BattlesBetElement.vue'
  import { computed, watch } from 'vue'

  const tournamentStore = useTournamentStore()

  // Computed
  const battlesGetPlayerCount = computed(() => {
    let count = 2
