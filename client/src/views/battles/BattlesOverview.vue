...
<script setup>
  import BattlesGameElement from '@/components/battles/BattlesGameElement.vue'
  import { computed } from 'vue'

  const tournamentStore = useTournamentStore()
  const { battlesGames, battlesHistory } = storeToRefs(tournamentStore)

  // Computed
  const battlesGetGames = computed(() => {
    let games = battlesGames

    if (battlesFilterSortGames === 'price') {
